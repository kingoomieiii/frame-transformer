...
def train_epoch(dataloader, pafnet, model, device, optimizer, accumulation_steps, progress_bar, lr_warmup=None, grad_scaler=None, use_wandb=True, step=0, model_dir="", save_every=20000):
    pafnet.eval()
    model.train()
...
def validate_epoch(dataloader, pafnet, model, device):
    model.eval()
    crit = nn.L1Loss()
...
def main():
    ...
    pafnet = FrameTransformer(in_channels=4, out_channels=4, channels=args.channels, dropout=args.dropout, n_fft=args.n_fft, num_heads=args.num_heads, expansion=args.expansion)
    model = FrameTransformer(in_channels=4, out_channels=2, channels=args.channels, dropout=args.dropout, n_fft=args.n_fft, num_heads=args.num_heads, expansion=args.expansion)
    ...
    while step < args.stages[-1]:
        ...
        train_loss_mag, step = train_epoch(train_dataloader, pafnet, model, device, optimizer, accum_steps, args.progress_bar, lr_warmup=scheduler, grad_scaler=grad_scaler, use_wandb=args.wandb, step=step, model_dir=args.model_dir)
        val_loss_mag = validate_epoch(val_dataloader, pafnet, model, device)
