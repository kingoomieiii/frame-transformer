...
This fork also makes use of vocal augmentations. This takes a random vocal spectrogram and adds it to an isntrumental spectrogram for training; the dataset takes a path to a directory with vocal spectrogram npz files (need to update the dataset generator code still, the new architecture expects unnormalied magnitudes with the normalization coefficient set in its own property in the npz file).
