...
Current experiment is with a semi-recursive transformer which consists of a transformer encoder repeated between 6 and N times (18 in current test) where the output is concatenated with the input and sent through a final 1x1 conv2d. Working surprisingly well in terms of validation loss after only an hour of training, so will be letting this one train; current model has 58,728,488 parameters and yet outperformed another variant with 800 million parameters after an hour of training vs the others ~2.5 hours (although this isn't a fair test obviously, would have to train the other out fully to see where it finishes). The benefits of selecting a random number of layers to apply at training time is of course a pretty sizable speed increase, as well as the ability for the end-user to reconfigure the neural network after training. Kinda think I may start tracking my weird experiments a little more rigorously and in sub-folders with source, hyperparameters, and results in case its helpful to anyone else at some point. Would be cool to start doing some open source research type stuff, one of my dreams at this point is truly democratizing AI and that starts with a distributed community effort focused on non-profit goals imo. 
