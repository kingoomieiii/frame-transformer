...
class FrameTransformer(nn.Module):
    def __init__(self, channels, n_fft=2048, feedforward_dim=512, num_heads=2, num_bands=2, num_encoders=1, num_decoders=1, cropsize=256, bias=False):
        ...
        self.dec4_transformer = nn.ModuleList([FrameTransformer2Decoder(channels * 8 + (num_heads*i), channels * 8, num_heads, num_bands, cropsize, n_fft, downsamples=4, feedforward_dim=feedforward_dim, bias=bias) for i in range(num_decoders)])
        self.dec4 = Decoder(channels * (6 + 8) + (num_heads*num_decoders), channels * 6, kernel_size=3, padding=1)

        self.dec3_transformer = nn.ModuleList([FrameTransformer2Decoder(channels * 6 + (num_heads*i), channels * 6, num_heads, num_bands, cropsize, n_fft, downsamples=3, feedforward_dim=feedforward_dim, bias=bias) for i in range(num_decoders)])
        self.dec3 = Decoder(channels * (4 + 6) + (num_heads*num_decoders), channels * 4, kernel_size=3, padding=1)

        self.dec2_transformer = nn.ModuleList([FrameTransformer2Decoder(channels * 4 + (num_heads*i), channels * 4, num_heads, num_bands, cropsize, n_fft, downsamples=2, feedforward_dim=feedforward_dim, bias=bias) for i in range(num_decoders)])
        self.dec2 = Decoder(channels * (2 + 4) + (num_heads*num_decoders), channels * 2, kernel_size=3, padding=1)

        self.dec1_transformer = nn.ModuleList([FrameTransformer2Decoder(channels * 2 + (num_heads*i), channels * 2, num_heads, num_bands, cropsize, n_fft, downsamples=1, feedforward_dim=feedforward_dim, bias=bias) for i in range(num_decoders)])
        self.dec1 = Decoder(channels * (1 + 2) + (num_heads*num_decoders), channels * 1, kernel_size=3, padding=1)

        self.out_transformer = nn.ModuleList([FrameTransformer2Decoder(channels + (num_heads*i), channels, num_heads, num_bands, cropsize, n_fft, downsamples=0, feedforward_dim=feedforward_dim, bias=bias) for i in range(num_decoders)])
        self.out = nn.Linear(channels + (num_heads*num_decoders), 2, bias=bias)
...
class FrameTransformer2Decoder(nn.Module):
    def __init__(self, channels, mem_channels, num_heads=1, num_bands=4, cropsize=256, n_fft=2048, feedforward_dim=2048, downsamples=0, bias=False, dropout=0.1):
        super(FrameTransformer2Decoder, self).__init__()
        ...
        self.bottleneck_linear = nn.Linear(channels, num_heads, bias=bias)
        self.bottleneck_norm = nn.BatchNorm2d(num_heads)
        self.mem_bottleneck_linear = nn.Linear(mem_channels, num_heads, bias=bias)
        self.mem_bottleneck_norm = nn.BatchNorm2d(num_heads)

        self.self_attn1 = MultiheadMultibandFrameAttention(num_heads, num_bands, bins, cropsize)
        self.enc_attn1 = MultiheadMultibandFrameAttention(num_heads, num_bands, bins, cropsize)
        self.norm1 = nn.LayerNorm(bins)
        self.dropout1 = nn.Dropout(dropout) if dropout > 0 else nn.Identity()

        self.conv1L = nn.Sequential(
            nn.Conv2d(bins, bins, kernel_size=(11,1), padding=(5,0), groups=bins, bias=bias),
            nn.Conv2d(bins, feedforward_dim * 2, kernel_size=1, padding=0, bias=bias))
        self.conv1R = nn.Sequential(
            nn.Conv2d(bins, bins, kernel_size=(7,1), padding=(3,0), groups=bins, bias=bias),
            nn.Conv2d(bins, feedforward_dim // 2, kernel_size=1, padding=0, bias=bias))
        self.norm2 = nn.LayerNorm(feedforward_dim * 2)
        self.conv2 = nn.Sequential(
            nn.Conv2d(feedforward_dim * 2, feedforward_dim * 2, kernel_size=(7,1), padding=(3,0), groups=feedforward_dim*2, bias=bias),
            nn.Conv2d(feedforward_dim * 2, bins, kernel_size=1, padding=0, bias=bias))
        self.norm3 = nn.LayerNorm(bins)
        self.dropout2 = nn.Dropout(dropout) if dropout > 0 else nn.Identity()

        self.self_attn2 = MultiheadMultibandFrameAttention(num_heads, num_bands, bins, cropsize)
        self.norm4 = nn.LayerNorm(bins)
        self.dropout3 = nn.Dropout(dropout) if dropout > 0 else nn.Identity()

        self.enc_attn2 = MultiheadMultibandFrameAttention(num_heads, num_bands, bins, cropsize)
        self.norm5 = nn.LayerNorm(bins)
        self.dropout4 = nn.Dropout(dropout) if dropout > 0 else nn.Identity()
    ...
    def __call__(self, x, mem):
        x = self.relu(self.bottleneck_norm(self.bottleneck_linear(x.transpose(1,3)).transpose(1,3))) # B,Ce,H,W
        mem = self.relu(self.mem_bottleneck_norm(self.mem_bottleneck_linear(mem.transpose(1,3)).transpose(1,3))) # B,Ce,H,W

        #b, ce, h, w = x.shape
        
        x = x.transpose(2,3) # b,ce,w,h
        mem = mem.transpose(2,3)

        hs = self.self_attn1(x)
        hm = self.enc_attn1(x, mem=mem)
        x = self.norm1(x + self.dropout1(hs + hm))

        hL = self.relu(self.conv1L(x.transpose(1,3)).transpose(1,3))
        hR = self.conv1R(x.transpose(1,3)).transpose(1,3)
        h = self.norm2(hL + F.pad(hR, (0, hL.shape[3]-hR.shape[3])))

        h = self.dropout2(self.conv2(h.transpose(1,3)).transpose(1,3))
        x = self.norm3(x + h)
...
class MultiheadMultibandFrameAttention(nn.Module):
    def __init__(self, num_heads, num_bands, bins, cropsize):
        super().__init__()
    ...
    def forward(self, x, mem=None):
        ...
        qk = torch.matmul(q,k) / math.sqrt(c)
        a = F.softmax(qk+p, dim=-1)
        a = torch.matmul(a,v).transpose(2,3).reshape(b,h,w,-1)
        o = self.o_proj(a)
        return o
