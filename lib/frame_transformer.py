...
class FrameTransformer(nn.Module):
    def __init__(self, channels, n_fft=2048, feedforward_dim=512, num_bands=8, num_encoders=1, num_decoders=1, cropsize=256, bias=False):
        ...
        self.dec0_transformer = nn.ModuleList([FrameTransformerDecoder(channels * 1 + i, channels * 1, num_bands, cropsize, n_fft, downsamples=0, feedforward_dim=feedforward_dim, bias=bias) for i in range(num_decoders)])
    ...
    def __call__(self, x):
        ...
        h = self.dec1(h, e1)
        for module in self.dec0_transformer:
