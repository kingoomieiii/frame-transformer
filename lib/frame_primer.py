...
class FramePrimer(nn.Module):
    def __init__(self, channels=2, n_fft=2048, feedforward_dim=512, num_bands=4, num_transformer_blocks=1, cropsize=1024, bias=False, out_activate=nn.Sigmoid(), dropout=0.1, pretraining=True):
        ...
        self.label_out = nn.Sequential(
            nn.Conv2d(channels + num_transformer_blocks, 1, kernel_size=1, padding=0, bias=False),
    ...
    def __call__(self, x):
        ...
        return torch.sigmoid(self.out(x.transpose(1,3)).transpose(1,3)), self.label_out(x).squeeze(-1).squeeze(-1)
