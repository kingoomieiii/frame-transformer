...
class Separator(object):
    ...
    def separate(self, X_spec, padding=None):
        ...
        xm = X_mag / X_mag.max()
        xp = (X_phase + np.pi) / (2 * np.pi)
        X_mag_pad = np.pad(np.concatenate((xm, xp), axis=0), ((0, 0), (0, 0), (pad_l, pad_r)), mode='constant')
...
def main():
    ...
    p.add_argument('--pretrained_model', '-P', type=str, default='model.pth') # https://mega.nz/file/61hCRSYD#fCtZH2DTul56HGVAF1i4W5PCDLtoEWZbypJWC9N089M
    ...
    p.add_argument('--num_attention_maps', type=int, default=15)
    ...
    model = FrameTransformerGenerator(in_channels=4, out_channels=2, channels=args.channels, expansion=args.expansion, n_fft=args.n_fft, dropout=args.dropout, num_heads=args.num_heads, num_attention_maps=args.num_attention_maps, num_bridge_layers=args.num_bridge_layers, latent_expansion=args.latent_expansion)
