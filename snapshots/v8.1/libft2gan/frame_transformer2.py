...
class FrameTransformerEncoder(nn.Module):
    def __init__(self, channels, out_channels, features, dropout=0.1, expansion=4, num_heads=8):
        ...
        self.conv1 = nn.Conv2d(channels + out_channels, (channels + out_channels) * expansion, kernel_size=3, padding=1, bias=False)
        self.conv2 = nn.Conv2d((channels + out_channels) * expansion, channels, kernel_size=3, padding=1, bias=False)
...
class FrameTransformerDecoder(nn.Module):
    def __init__(self, channels, out_channels, features, dropout=0.1, expansion=4, num_heads=8, has_prev_skip=True):
        ...
        self.conv1 = nn.Conv2d(channels + out_channels * 2, (channels + out_channels * 2) * expansion, kernel_size=3, padding=1, bias=False)
        self.conv2 = nn.Conv2d((channels + out_channels * 2) * expansion, channels, kernel_size=3, padding=1, bias=False)
