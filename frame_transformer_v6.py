...
from frame_transformer_prenorm import FrameLinear
from rotary_embedding_torch import RotaryEmbedding

class FrameTransformer(nn.Module):
    def __init__(self, in_channels=2, channels=2, dropout=0.1, n_fft=2048, num_heads=4, expansion=2, num_attention_maps=[2,4,8,16,24,32]):
...
class FrameTransformerEncoder(nn.Module):
    def __init__(self, in_channels, out_channels, features, expansion=4, num_heads=8, dropout=0.1):
        ...
        self.linear1 = FrameLinear(features, expansion)
        self.linear2 = FrameLinear(expansion, features)
...
class MultichannelMultiheadAttention(nn.Module):
    def __init__(self, channels, num_heads, features, kernel_size=3, padding=1, separable=True):
        ...
        self.out_proj = FrameLinear(features, features)
