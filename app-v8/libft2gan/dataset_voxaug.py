...
from libft2gan.dataset_utils import apply_channel_drop, apply_dynamic_range_mod, apply_multiplicative_noise, apply_random_eq, apply_stereo_spatialization, apply_time_stretch, apply_pitch_shift, apply_dynamic_range_mod_pair, apply_random_eq_pair, apply_channel_drop_pair, apply_stereo_spatialization_pair
import librosa
...
class VoxAugDataset(torch.utils.data.Dataset):
    ...
    def _augment_pair(self, X, Y, c):
        ...
        augmentations = [
            (0.02, apply_channel_drop_pair, { "channel": self.random.randint(0,2), "alpha": self.random.uniform(0,1) }),
            #(0.2, apply_dynamic_range_mod_pair, { "c": c, "threshold": self.random.uniform(0,1), "gain": self.random.uniform(0,0.125), }),
            # (0.2, apply_random_eq_pair, { "min": self.random.uniform(0.5, 1), "max": self.random.uniform(1, 1.5), }),
