...
from libft2gan.frame_transformer import FrameTransformer
...
def main():
    ...
    p.add_argument('--checkpoint', type=str, default=None)#'model.pth')
    p.add_argument('--mixed_precision', type=str, default='true')
    p.add_argument('--use_swa', type=str, default="false")
    p.add_argument('--learning_rate', '-l', type=float, default=1e-7)
    ...
    p.add_argument('--model_dir', type=str, default='D://')
    p.add_argument('--instrumental_lib', type=str, default="D://cs2048_sr44100_hl1024_nf2048_of0_PAIRS")
    p.add_argument('--vocal_lib', type=str, default=None)
    ...
    p.add_argument('--decay_steps', type=int, default=100000)
    ...
    args.vocal_lib = [p for p in args.vocal_lib.split('|')] if args.vocal_lib is not None else args.vocal_lib
    ...
    # groups = [
    #     # { "params": filter(lambda p: p.requires_grad, model.enc1_transformer.parameters()), "lr": args.learning_rate * 0.1 },
    #     # { "params": filter(lambda p: p.requires_grad, model.enc2_transformer.parameters()), "lr": args.learning_rate * 0.2 },
    #     # { "params": filter(lambda p: p.requires_grad, model.enc3_transformer.parameters()), "lr": args.learning_rate * 0.3 },
    #     # { "params": filter(lambda p: p.requires_grad, model.enc4_transformer.parameters()), "lr": args.learning_rate * 0.4 },
    #     # { "params": filter(lambda p: p.requires_grad, model.enc5_transformer.parameters()), "lr": args.learning_rate * 0.5 },
    #     # { "params": filter(lambda p: p.requires_grad, model.enc6_transformer.parameters()), "lr": args.learning_rate * 0.6 },
    #     # { "params": filter(lambda p: p.requires_grad, model.enc7_transformer.parameters()), "lr": args.learning_rate * 0.7 },
    #     # { "params": filter(lambda p: p.requires_grad, model.enc8_transformer.parameters()), "lr": args.learning_rate * 0.8 },
    #     { "params": filter(lambda p: p.requires_grad, model.enc9_transformer.parameters()), "lr": args.learning_rate * 0.1 },
    #     { "params": filter(lambda p: p.requires_grad, model.dec8_transformer.parameters()), "lr": args.learning_rate * 0.2 },
    #     { "params": filter(lambda p: p.requires_grad, model.dec7_transformer.parameters()), "lr": args.learning_rate * 0.3 },
    #     { "params": filter(lambda p: p.requires_grad, model.dec6_transformer.parameters()), "lr": args.learning_rate * 0.4 },
    #     { "params": filter(lambda p: p.requires_grad, model.dec5_transformer.parameters()), "lr": args.learning_rate * 0.5 },
    #     { "params": filter(lambda p: p.requires_grad, model.dec4_transformer.parameters()), "lr": args.learning_rate * 0.6 },
    #     { "params": filter(lambda p: p.requires_grad, model.dec3_transformer.parameters()), "lr": args.learning_rate * 0.7 },
    #     { "params": filter(lambda p: p.requires_grad, model.dec2_transformer.parameters()), "lr": args.learning_rate * 0.8 },
    #     { "params": filter(lambda p: p.requires_grad, model.dec1_transformer.parameters()), "lr": args.learning_rate * 0.9 },
    # ]
    ...
    scheduler_p = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=3, threshold=0.0001, threshold_mode='abs')
