...
class FrameTransformer(nn.Module):
    def __init__(self, in_channels=2, out_channels=2, channels=2, dropout=0.1, n_fft=2048, num_heads=4, expansion=4, transformer_channels=[2,4,8,16,32,1536,3072]):
        ...
        self.enc5 = FrameEncoder(channels * 6 + transformer_channels[4], channels * 8, self.max_bin // 8)
        ...
        self.enc6 = FrameEncoder(channels * 8 + transformer_channels[5], channels * 10, self.max_bin // 16)
        ...
        self.dec5 = FrameDecoder(channels * 10, transformer_channels[6], transformer_channels[5], channels * 8, self.max_bin // 16)
    ...
    def __call__(self, x):
        ...
        e6, a6 = self.enc6_transformer(self.enc6(e5))
...
class FrameTransformerDecoder(nn.Module):
    def __init__(self, channels, out_channels, features, dropout=0.1, expansion=4, num_heads=8):
        ...
        self.conv1 = MultichannelLinear(out_channels, out_channels, features, features * expansion, depthwise=True)
        self.conv2 = MultichannelLinear(out_channels, out_channels, features * expansion, features)
...
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, features, downsample=False):
        ...
        self.conv1 = nn.Conv2d(in_channels * (2 if downsample else 1), out_channels, kernel_size=3, padding=1, bias=False)
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1, stride=(2,1) if downsample else 1, bias=False)
        self.identity = nn.Conv2d(in_channels * (2 if downsample else 1), out_channels, kernel_size=1, padding=0, stride=(2,1) if downsample else 1, bias=False) if in_channels != out_channels or downsample else nn.Identity()
...
class FrameDecoder(nn.Module):
    def __init__(self, in_feature_maps, in_attention_maps, in_skip_attention_maps, out_channels, features):
        super(FrameDecoder, self).__init__()

        self.in_feature_maps = in_feature_maps
        self.in_attention_maps = in_attention_maps
        self.in_skip_attention_maps = in_skip_attention_maps
        self.upsample = nn.Upsample(scale_factor=(2,1), mode='bilinear', align_corners=True)
        self.body = ResBlock((in_feature_maps + in_attention_maps) // 2 + in_skip_attention_maps + out_channels, out_channels, features)

    def __call__(self, x, skip):
        s = torch.cat((x[:, :self.in_feature_maps // 2, :, :], x[:, self.in_feature_maps:self.in_feature_maps+(self.in_attention_maps//2), :, :], x[:, (self.in_feature_maps // 2):self.in_feature_maps, :, :], x[:, (self.in_feature_maps + (self.in_attention_maps // 2)):, :, :]), dim=1)
