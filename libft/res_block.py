...
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, features, downsample=False, expansion=1, kernel_size=3, padding=1):
        super(ResBlock, self).__init__()

        self.norm = MultichannelLayerNorm(in_channels, features)
        self.conv1 = nn.Conv2d(in_channels, out_channels * expansion, kernel_size=kernel_size, padding=padding, bias=False)
        self.conv2 = nn.Conv2d(out_channels * expansion, out_channels, kernel_size=kernel_size, padding=padding, bias=False, stride=2 if downsample else 1)
