...
class FramePrimer(nn.Module):
    def __init__(self, in_channels=2, channels=2, depth=4, num_transformer_encoders=0, num_transformer_decoders=0, dropout=0.1, n_fft=2048, cropsize=2048, num_bands=8, feedforward_dim=2048, bias=True, scale_factor=1, num_res_blocks=1):
        ...
        self.decoder = [FrameDecoder(channels * scale_factor + num_transformer_decoders, channels, kernel_size=3, padding=1, cropsize=cropsize, downsamples=0, num_res_blocks=num_res_blocks, upsample=False)]
        ...
        self.transformer_decoder = [nn.ModuleList([FramePrimerDecoder(channels * scale_factor + i, channels + num_transformer_encoders, num_bands=num_bands, cropsize=cropsize, n_fft=n_fft, downsamples=0, feedforward_dim=feedforward_dim, bias=bias, dropout=dropout) for i in range(num_transformer_decoders)])]

        for i in range(depth - 1):
            self.encoder.append(FrameEncoder(channels * scale_factor * (i + 1) + num_transformer_encoders, channels * scale_factor * (i + 2), stride=2, cropsize=cropsize, downsamples=i, num_res_blocks=num_res_blocks))
            self.decoder.append(FrameDecoder(channels * scale_factor * (i + 2) + num_transformer_decoders + (num_transformer_encoders if i == depth - 2 else 0) + channels * scale_factor * (i + 1) + num_transformer_encoders, channels * scale_factor * (i + 1), cropsize=cropsize, downsamples=i, num_res_blocks=num_res_blocks))
...
class FramePrimer2(nn.Module):
    def __init__(self, in_channels=2, channels=2, depth=4, scale_factor=1, num_transformer_blocks=0, feedforward_expansion=4, dropout=0.1, n_fft=2048, cropsize=2048, num_bands=16, bias=True, num_res_blocks=1):
        ...
        print(channels * scale_factor * ((depth - 2) + 2))
    ...
    def forward(self, x):
        ...
        x = x.reshape(b,w,h,c).transpose(1,3)
