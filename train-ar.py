...
from causal_frame_transformer import FrameTransformer
...
def train_epoch(dataloader, model, device, optimizer, accumulation_steps, progress_bar, mixup_rate, mixup_alpha, lr_warmup=None, grad_scaler=None, use_wandb=True, step=0):
    ...
    for itr, (src, tgt) in enumerate(pbar):
        ...
        with torch.cuda.amp.autocast_mode.autocast(enabled=grad_scaler is not None):
            pred = torch.sigmoid(model(src, F.pad(tgt, (1,0), value=1)[:, :, :, :-1]))

        l1_loss = crit(src * pred, tgt) / accumulation_steps
    ...
    with torch.no_grad():
        for src, tgt in tqdm(dataloader):
            ...
            if torch.logical_or(mag_loss.isnan(), mag_loss.isinf()):
                ...
                sum_loss += mag_loss.item() * len(src)
