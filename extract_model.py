...
def load_from_huggingface(transformer, name='bert-large-uncased'):
    ...
    for layer in pre.encoder.layer:
        if idx < len(transformer):
            ...
            transformer[idx].attn.o_proj.weight_pw.data = expand_weight(layer.attention.output.dense.weight, channels)
            transformer[idx].attn.o_proj.bias_pw.data = expand_linear_bias(layer.attention.output.dense.bias, channels)
...
def main():
    ...
    p.add_argument('--embedding', type=int, default=1024)
