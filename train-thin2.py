...
from frame_transformer_thin2 import FrameTransformer
...
def main():
    ...
    p.add_argument('--base_channels', type=int, default=8)
    p.add_argument('--base', type=str, default='/media/ben/internal-nvme-b//models/base.pth')
    ...
    p.add_argument('--batch_sizes', type=str, default='4,8,3,1')
    p.add_argument('--accumulation_steps', '-A', type=str, default='2,1,2,4')
    ...
    if args.base is not None:
        base = FrameTransformer(in_channels=2, out_channels=2, channels=args.base_channels, expansion=args.feedforward_expansion, n_fft=args.n_fft, dropout=args.dropout, num_heads=args.num_heads)
        base.load_state_dict(torch.load(f'{args.base}'))
        model.load_from_other(base)
        model.to(device)
