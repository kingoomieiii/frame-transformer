...
from libft2gan.dataset_utils import apply_channel_drop, apply_dynamic_range_mod, apply_harmonic_distortion, apply_multiplicative_noise, apply_random_eq, apply_stereo_spatialization, apply_time_stretch, apply_pitch_shift, apply_random_phase_noise, apply_time_masking, apply_frequency_masking, apply_emphasis
...
class VoxAugDataset(torch.utils.data.Dataset):
    ...
    def _get_vocals(self, idx):
        ...
        augmentations = [
            ...
            (0.2, apply_dynamic_range_mod, { "c": Vc, "threshold": np.random.uniform(0, 0.5), "ratio": np.random.randint(1,2) }),
            (0.2, apply_harmonic_distortion, { "c": Vc, "num_harmonics": np.random.randint(1, 5), "gain": np.random.uniform(0, 0.5), "n_fft": self.n_fft, "hop_length": self.hop_length }),
            ...
            (0.2, apply_stereo_spatialization, { "alpha": np.random.uniform(0, 2) }),
            (0.2, apply_pitch_shift, { "c": Vc, "n_fft": self.n_fft, "hop_length": self.hop_length, "sr": self.sr, "n_steps": np.random.uniform(-3, 3) }),
            ...
            (0.2, apply_emphasis, { "c": Vc, "emphasis_coef": np.random.uniform(0.9, 1), "n_fft": self.n_fft, "hop_length": self.hop_length }),
    ...
    def _augment_instruments(self, X, c):
        ...
        augmentations = [
            (0.03, apply_channel_drop, { "channel": 0}),
            (0.03, apply_channel_drop, { "channel": 1}),
            (0.5, apply_random_eq, { "min": np.random.uniform(0.8,1), "max": np.random.uniform(1, 1.2) }),
            (0.5, apply_stereo_spatialization, { "alpha": np.random.uniform(0.8, 1.2) })
        ]
