...
from libft2gan.frame_transformer_ar import FrameTransformerGenerator
...
def validate_epoch(dataloader, model, device, max_bin=0, use_wandb=False, predict_mask=True, predict_phase=False, num_quantizer_levels=256):
    ...
    with torch.no_grad():
        for itr, (XW, YW, c) in enumerate(tqdm(dataloader)):
            XW = XW.to(device)
            YW = YW.to(device)
            c = c.to(device).unsqueeze(-1)
            YS = to_spec(YW)
            YS = torch.abs(YS)
            XS = torch.abs(to_spec(XW)) 
...
def main():
    ...
    p.add_argument('--num_tempograms', type=int, default=1)
    p.add_argument('--num_octave_channels', type=int, default=1)
