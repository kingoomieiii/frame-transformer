...
def train_epoch(dataloader, model, device, optimizer, accumulation_steps, progress_bar, lr_warmup=None, grad_scaler=None, step=0, max_bin=0, predict_mask=True, predict_phase=False):
    model.train()
...
def validate_epoch(dataloader, model, device, max_bin=0, predict_mask=True, predict_phase=False):
    model.train()
    crit = nn.L1Loss()
...
def main():
    ...
    while step < args.stages[-1]:
        ...
        train_loss_mag, step = train_epoch(train_dataloader, generator, device, optimizer=optimizer_gen, accumulation_steps=accum_steps, progress_bar=args.progress_bar, lr_warmup=scheduler_gen, grad_scaler=grad_scaler_gen, step=step, max_bin=args.n_fft // 2, predict_mask=args.predict_mask, predict_phase=args.predict_phase)
        ...
        if args.world_rank == 0:
            ...
            torch.save(generator.state_dict(), f'{model_path}.stg0.{"phase" if args.predict_phase else "mag"}.pth')
