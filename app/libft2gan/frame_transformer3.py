...
class FrameTransformerGenerator(nn.Module):
    def __init__(self, in_channels=2, out_channels=2, channels=2, dropout=0.1, n_fft=2048, num_heads=4, expansion=4, latent_expansion=4, num_bridge_layers=9, num_attention_maps=1):
        ...
        self.enc9_transformer = nn.Sequential(*[ConvolutionalTransformerEncoder(channels * 16 * 4, dropout=dropout, expansion=latent_expansion, num_heads=num_heads, kernel_size=1, padding=0) for _ in range(num_bridge_layers)])
...
class FrameTransformerDecoder(nn.Module):
    ...
    def forward(self, x, skip, prev_qk1=None, prev_qk2=None, skip_qk=None):
        ...
        z = self.norm1(h)
        za, prev_qk1 = self.attn1(z, prev_qk=prev_qk1)
        ...
        zb, prev_qk2 = self.attn2(self.norm2(z), mem=skip, prev_qk=prev_qk2)
