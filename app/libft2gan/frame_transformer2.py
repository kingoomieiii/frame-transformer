...
class FrameTransformerGenerator(nn.Module):
    ...
    def forward(self, x):
        ...
        e8 = self.enc8(e7)
        e8, qk8 = self.enc8_transformer(e8, prev_qk=qk7)
...
class FrameTransformerEncoder(nn.Module):
    def __init__(self, channels, out_channels, features, dropout=0.1, expansion=4, num_heads=8):
        ...
        self.glu = nn.Sequential(
            MultichannelLinear(channels, channels, features, features * 2),
            nn.GLU(dim=-2))
        
        self.norm2 = MultichannelLayerNorm(channels, features)
        self.conv1a = MultichannelLinear(channels, channels, features, features)
        self.conv1b = FrameConv(channels, channels, features, features, kernel_size=3, padding=1, groups=channels)

        self.norm3 = MultichannelLayerNorm(channels, features)
        self.conv2 = FrameConv(channels, channels, features, features, kernel_size=9, padding=4, groups=channels)
...
class FrameTransformerDecoder(nn.Module):
    def __init__(self, channels, out_channels, features, dropout=0.1, expansion=4, num_heads=8, has_prev_skip=True):
        ...
        self.norm1 = MultichannelLayerNorm(channels, features)
        self.attn1a = MultichannelMultiheadAttention2(channels, out_channels, num_heads, features, kernel_size=3, padding=1)
        self.attn1b = MultichannelMultiheadAttention2(channels, out_channels, num_heads, features, kernel_size=3, padding=1)

        self.norm2 = MultichannelLayerNorm(channels, features)
        self.conv1a = FrameConv(channels, channels, features, features, kernel_size=11, padding=5, groups=channels)
        self.conv1b = FrameConv(channels, channels, features, features, kernel_size=7, padding=3, groups=channels)
        self.norm3 = MultichannelLayerNorm(channels, features)
        self.conv2 = FrameConv(channels, channels, features, features, kernel_size=7, padding=3, groups=channels)

        self.norm4 = MultichannelLayerNorm(channels, features)
        self.attn2 = MultichannelMultiheadAttention2(channels, out_channels, num_heads, features, kernel_size=3, padding=1)

        self.norm5 = MultichannelLayerNorm(channels, features)
        self.attn3 = MultichannelMultiheadAttention2(channels, out_channels, num_heads, features, kernel_size=3, padding=1)
    ...
    def forward(self, x, skip, prev_qk1=None, prev_qk2=None, skip_qk=None):
        ...
        h = x + self.dropout(za) + self.dropout(zb)
