...
from libft.frame_transformer_vae import FrameTransformer
from libft.signal_loss import sdr_loss
import libft.signal_loss as signal_loss
...
def train_epoch(dataloader, model, device, optimizer, accumulation_steps, progress_bar, lr_warmup=None, grad_scaler=None, use_wandb=True, step=0, model_dir="", save_every=20000):
    ...
    for itr, (X, Y) in enumerate(pbar):
        ...
        kld_loss = signal_loss.kl_divergence(mu, logvar) / accumulation_steps
        accum_loss = mae_loss + kld_loss
        ...
        if (itr + 1) % accumulation_steps == 0:
            if progress_bar:
                pbar.set_description(f'{step}: {str(batch_mag_loss.item())}|{str(kld_loss.item())}|{pavg.item()}|{pmin.item()}')
