...
class FrameTransformer(nn.Module):
    def __init__(self, in_channels=2, channels=2, dropout=0.1, n_fft=2048, num_heads=4, expansion=2, num_attention_maps=[8,8,8,8,8,8,8], out=True):
        super(FrameTransformer, self).__init__()
        
        self.max_bin = n_fft // 2
        self.output_bin = n_fft // 2 + 1

        self.enc0_transformer = FrameTransformerEncoder(in_channels, num_attention_maps[0], self.max_bin, dropout=dropout, expansion=expansion, num_heads=num_heads)

        self.enc1 = FrameEncoder(in_channels + num_attention_maps[0], channels, self.max_bin, downsample=False)
        self.enc1_transformer = FrameTransformerEncoder(channels, num_attention_maps[1], self.max_bin, dropout=dropout, expansion=expansion, num_heads=num_heads)

        self.enc2 = FrameEncoder(channels * 1 + num_attention_maps[1], channels * 2, self.max_bin)
        self.enc2_transformer = FrameTransformerEncoder(channels * 2, num_attention_maps[2], self.max_bin // 2, dropout=dropout, expansion=expansion, num_heads=num_heads)

        self.enc3 = FrameEncoder(channels * 2 + num_attention_maps[2], channels * 4, self.max_bin // 2)
        self.enc3_transformer = FrameTransformerEncoder(channels * 4, num_attention_maps[3], self.max_bin // 4, dropout=dropout, expansion=expansion, num_heads=num_heads)

        self.enc4 = FrameEncoder(channels * 4 + num_attention_maps[3], channels * 6, self.max_bin // 4)
        self.enc4_transformer = FrameTransformerEncoder(channels * 6, num_attention_maps[4], self.max_bin // 8, dropout=dropout, expansion=expansion, num_heads=num_heads)

        self.enc5 = FrameEncoder(channels * 6 + num_attention_maps[4], channels * 8, self.max_bin // 8)
        self.enc5_transformer = FrameTransformerEncoder(channels * 8, num_attention_maps[5], self.max_bin // 16, dropout=dropout, expansion=expansion, num_heads=num_heads)

        self.enc6 = FrameEncoder(channels * 8 + num_attention_maps[5], channels * 10, self.max_bin // 16)
        self.enc6_transformer = FrameTransformerEncoder(channels * 10, num_attention_maps[6], self.max_bin // 32, dropout=dropout, expansion=expansion, num_heads=num_heads)

        self.dec5 = FrameDecoder(channels * (10 + 8) + num_attention_maps[6] + num_attention_maps[5], channels * 8, self.max_bin // 16)
        self.dec5_transformer = FrameTransformerEncoder(channels * 8, num_attention_maps[5], self.max_bin // 16, dropout=dropout, expansion=expansion, num_heads=num_heads)

        self.dec4 = FrameDecoder(channels * (8 + 6) + num_attention_maps[5] + num_attention_maps[4], channels * 6, self.max_bin // 8)
        self.dec4_transformer = FrameTransformerEncoder(channels * 6, num_attention_maps[4], self.max_bin // 8, dropout=dropout, expansion=expansion, num_heads=num_heads)

        self.dec3 = FrameDecoder(channels * (6 + 4) + num_attention_maps[4] + num_attention_maps[3], channels * 4, self.max_bin // 4)
        self.dec3_transformer = FrameTransformerEncoder(channels * 4, num_attention_maps[3], self.max_bin // 4, dropout=dropout, expansion=expansion, num_heads=num_heads)

        self.dec2 = FrameDecoder(channels * (4 + 2) + num_attention_maps[3] + num_attention_maps[2], channels * 2, self.max_bin // 2)
        self.dec2_transformer = FrameTransformerEncoder(channels * 2, num_attention_maps[2], self.max_bin // 2, dropout=dropout, expansion=expansion, num_heads=num_heads)

        self.dec1 = FrameDecoder(channels * (2 + 1) + num_attention_maps[2] + num_attention_maps[1], channels * 1, self.max_bin)
        self.dec1_transformer = FrameTransformerEncoder(channels * 1, num_attention_maps[1], self.max_bin // 1, dropout=dropout, expansion=expansion, num_heads=num_heads)

        self.out = nn.Parameter(torch.empty(in_channels, channels + num_attention_maps[1])) if out else None

        if self.out is not None:
            nn.init.uniform_(self.out, a=-1/math.sqrt(channels + num_attention_maps[1]), b=1/math.sqrt(channels + num_attention_maps[1]))

    def __call__(self, x):
        t, prev_qk = self.enc0_transformer(x)
        ...
        d5 = self.dec5(e6, e5)
        t, prev_qk = self.dec5_transformer(d5, prev_qk=prev_qk)
        d5 = torch.cat((d5, t), dim=1)

        d4 = self.dec4(d5, e4)
        t, prev_qk = self.dec4_transformer(d4, prev_qk=prev_qk)
        d4 = torch.cat((d4, t), dim=1)

        d3 = self.dec3(d4, e3)
        t, prev_qk = self.dec3_transformer(d3, prev_qk=prev_qk)
        d3 = torch.cat((d3, t), dim=1)

        d2 = self.dec2(d3, e2)
        t, prev_qk = self.dec2_transformer(d2, prev_qk=prev_qk)
        d2 = torch.cat((d2, t), dim=1)

        d1 = self.dec1(d2, e1)
        t, prev_qk = self.dec1_transformer(d1, prev_qk=prev_qk)
...
class FrameTransformerEncoder(nn.Module):
    def __init__(self, in_channels, out_channels, features, expansion=4, num_heads=8, dropout=0.1, depthwise=False):
        super().__init__()
