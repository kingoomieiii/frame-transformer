...
from dataset_voxnoise import VoxNoiseDataset
from dataset_voxaug import VoxAugDataset

from lib import nets as nets
from frame_transformer_v4 import FrameTransformer
from frame_transformer_v4b import FrameTransformer as ConvFrameTransformer
...
def train_epoch(dataloader, model, device, optimizer, accumulation_steps, progress_bar, lr_warmup=None, grad_scaler=None, use_wandb=True, step=0):
    ...
    for itr, (X, Y) in enumerate(pbar):
        ...
        l1_loss = (crit(pred, Y) * 0.8 + crit(aux, Y) * 0.2) / accumulation_steps
...
def validate_epoch(dataloader, remover, model, device):
    model.eval()
    crit = nn.L1Loss()
    sum_loss = 0
    ...
    with torch.no_grad():
        for X, Y in dataloader:
            ...
            mask = torch.sigmoid(remover(X))
            pred = model(X * mask)
            mag_loss = crit(pred, Y)
...
def main():
    ...
    p.add_argument('--pretrained_remover', type=str, default=None)
    p.add_argument('--channels', type=int, default=32)
    p.add_argument('--feedforward_expansion', type=int, default=4)
    p.add_argument('--num_heads', type=int, default=8)
    p.add_argument('--dropout', type=float, default=0.1)

    p.add_argument('--cropsizes', type=str, default='256,768')
    p.add_argument('--steps', type=str, default='400000,500000')
    p.add_argument('--epochs', type=str, default='80,90')
    p.add_argument('--batch_sizes', type=str, default='12,1')
    p.add_argument('--accumulation_steps', '-A', type=str, default='1,8')
    ...
    train_dataset = VoxNoiseDataset(
        ...
        vocal_path=[
            ...
            "H://cs2048_sr44100_hl1024_nf2048_of0_VOCALS",
        ],
        is_validation=False,
    ...
    remover = FrameTransformer(in_channels=2, channels=args.channels, expansion=args.feedforward_expansion, n_fft=args.n_fft, dropout=args.dropout, num_heads=args.num_heads)
    model = nets.CascadedNet(args.n_fft, 32, 256)
    #model = ConvFrameTransformer(2, 16, 0.1, num_heads=8, expansion=4)

    if args.pretrained_model is not None:
        model.load_state_dict(torch.load(args.pretrained_model, map_location=device))

    if args.pretrained_remover is not None:
        remover.load_state_dict(torch.load(args.pretrained_remover, map_location=device))
    ...
    for epoch in range(args.curr_epoch, num_epochs):
        ...
        val_loss = validate_epoch(val_dataloader, remover, model, device)
        val_loss2 = validate_epoch(val_dataloader2, remover, model, device)
        ...
        model_path = f'{args.model_dir}models/{wandb.run.name if args.wandb else "local"}.{epoch}.corrector.pth'
