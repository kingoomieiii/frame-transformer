...
def main():
    ...
    p.add_argument('--warmup_steps', type=int, default=32)
    ...
    decay_steps = len(train_dataloader) - warmup_steps
    ...
    train_loss = train_epoch(train_dataloader, model, device, optimizer, args.accum_steps, args.progress_bar, lr_warmup=scheduler, grad_scaler=grad_scaler, use_wandb=args.wandb, step=step, model_dir=args.model_dir)
    ...
    Yr_spec = spec_utils.wave_to_spectrogram(Yr, args.hop_length, args.n_fft)
    Y_spec, V_spec, _ = separator.separate(X_spec, Yr_spec)
